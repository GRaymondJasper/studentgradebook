...
class StudentGradesApp:
    def __init__(self, root):
        ...
        self.bg_image = Image.open("login.jpg")  # Make sure to replace with your image path
        ...
        self.bg_label.place(relwidth=1, relheight=1)  # This makes the image fill the entire window
    ...
    def login(self):
        ...
        if username == USERNAME and password == PASSWORD:
            ...
            self.open_dashboard_page()  # Open dashboard after successful login
    ...
    def open_dashboard_page(self):
        ...
        # Set background image for dashboard window
        ...
        # Show only the total students count
    ...
    def open_search_page(self):
        ...
        # Set background image for search window
        ...
        self.marquee_text = self.marquee_text + " " * 20  # Add extra spaces for smooth scrolling
        self.move_marquee()
        ...
        # Button for saving PDF with hover effect
    ...
    def move_marquee(self):
        ...
        self.marquee_label.after(300, self.move_marquee)  # Adjust speed for slower movement (300 ms)

    def add_hover_effect(self, widget):
        def on_enter(event):
            widget['bg'] = "#45a049"  # Darker green when hovering
        def on_leave(event):
            widget['bg'] = "#4CAF50"  # Original green
    ...
    def save_as_pdf(self):
        try:
            ...
            last_student = None  # Track the last student's name to avoid repetition

            for idx, row in self.search_result.iterrows():
                # Print the student's name only once per student (if it's different from the last student)
                if row['studentname'] != last_student:
                    pdf.cell(50, 10, row['studentname'], border=1, align="C")
                    last_student = row['studentname']
                else:
                    pdf.cell(50, 10, "", border=1, align="C")  # Leave the cell empty for repeating student
